...
GLuint Shader::vbo_grenade_particle;
GLuint Shader::ebo_grenade_particle;
GLuint Shader::vbo_fire_particle;
GLuint Shader::ebo_fire_particle;
GLuint Shader::vbo_ice_particle;
GLuint Shader::ebo_ice_particle;

Shader::Shader(const char* vertPath, const char* fragPath, bool isSprite, int particleType) {
	if (!setupSprite)
	{
		makeSprite();
		makeGrenadeParticles();
		makeFireParticles();
		makeIceParticles();
	}
	...
	{
		SetAttributes_particle(particleType);
	}
}
...
void Shader::SetAttributes_particle(int particleType)
{

	glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);
	glDepthMask(GL_TRUE); // draw particles with writing to depth buffer

	if (particleType == 1) {
		glBindBuffer(GL_ARRAY_BUFFER, vbo_grenade_particle);
		glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, ebo_grenade_particle);
	}
	if (particleType == 2) {
		glBindBuffer(GL_ARRAY_BUFFER, vbo_fire_particle);
		glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, ebo_fire_particle);
	}
	if (particleType == 3) {
		glBindBuffer(GL_ARRAY_BUFFER, vbo_ice_particle);
		glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, ebo_ice_particle);
	}
...
{
	glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);
	glDepthMask(GL_TRUE); // draw sprites with writing to depth buffer
...
void Shader::makeGrenadeParticles()
{
	...
	for (int i = 0; i < 4000; i++)
	{
		if (i % 4 == 0)
		{
			theta = (2*(rand() % 10000) / 10000.0f -1.0f)*1.57f;//(6.28 * (rand() % 1000) / 1000.0f);
			r = 0.0f + 0.8f * (rand() % 10000) / 10000.0f;
			tmod = 0.0f * (rand() % 10000) / 10000.0f;
		}
		// position
...
void Shader::makeFireParticles()
{
	...
	GLfloat particleatt[4 * 400 * vertex_attr];
	float theta, r, tmod;
	tmod = 0.0f;
	theta = 0.0f;
	r = 0.0f;
	for (int i = 0; i < 4 * 400; i++) {
		if (i % 4 == 0) {
			...
			r = 0.0f + 1.0 * (rand() % 10000) / 10000.0f;
			tmod = 0.5f * (rand() % 10000) / 10000.0f;
		}
	...
	// Create buffer for vertices
	glGenBuffers(1, &vbo_fire_particle);
	glBindBuffer(GL_ARRAY_BUFFER, vbo_fire_particle);
	glBufferData(GL_ARRAY_BUFFER, sizeof(particleatt), particleatt, GL_STATIC_DRAW);

	// Create buffer for faces (index buffer)
	glGenBuffers(1, &ebo_fire_particle);
	glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, ebo_fire_particle);
	glBufferData(GL_ELEMENT_ARRAY_BUFFER, sizeof(manyface), manyface, GL_STATIC_DRAW);

}

void Shader::makeIceParticles()
{
	...
	for (int i = 0; i < 4 * 400; i++) {
		if (i % 4 == 0) {
			theta = (1.57 * (rand() % 9) / 9.0f) + 2.34f;//(2*(rand() % 10000) / 10000.0f -1.0f)*0.13f;
			r = 1.0f + 1.0 * (rand() % 10000) / 10000.0f;
			tmod = 0.5f * (rand() % 10000) / 10000.0f;
		}
	...
	// Create buffer for vertices
	glGenBuffers(1, &vbo_ice_particle);
	glBindBuffer(GL_ARRAY_BUFFER, vbo_ice_particle);
	glBufferData(GL_ARRAY_BUFFER, sizeof(particleatt), particleatt, GL_STATIC_DRAW);

	// Create buffer for faces (index buffer)
	glGenBuffers(1, &ebo_ice_particle);
	glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, ebo_ice_particle);
	glBufferData(GL_ELEMENT_ARRAY_BUFFER, sizeof(manyface), manyface, GL_STATIC_DRAW);
